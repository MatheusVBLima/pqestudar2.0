...
-- Drop índices existentes se houver
DROP INDEX IF EXISTS public.idx_news_published_at;
DROP INDEX IF EXISTS public.idx_news_category;
DROP INDEX IF EXISTS public.idx_news_title_hash;
DROP INDEX IF EXISTS public.idx_news_embedding;
...
INSERT INTO public.news_generator_config (id)
SELECT gen_random_uuid()
WHERE NOT EXISTS (SELECT 1 FROM public.news_generator_config LIMIT 1);
...
DROP POLICY IF EXISTS "Admins can view config" ON public.news_generator_config;
DROP POLICY IF EXISTS "Admins can update config" ON public.news_generator_config;
DROP POLICY IF EXISTS "Admins can view logs" ON public.news_generation_logs;
DROP POLICY IF EXISTS "Admins can insert logs" ON public.news_generation_logs;
...
DROP TRIGGER IF EXISTS update_config_updated_at ON public.news_generator_config;
