...
const Noticias = () => {
  ...
  // Load initial AI-generated news on mount
  useEffect(() => {
    const initialNews = AINewsService.generateNews(8);
    setNoticias(initialNews);
    // Store news for detail page access
    NewsStorageService.storeNews(initialNews);
  }, []);
  ...
  const noticiasFiltradas = filtroAtivo === "Todas" 
    ? noticias 
                  ...
                  <TooltipContent className="max-w-sm p-4">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        Nosso sistema automatiza a busca por notícias relevantes sobre educação, 
                        validando informações através de múltiplas fontes confiáveis. Filtramos 
                        conteúdo por relevância, veracidade e importância, garantindo que você 
                        receba apenas as notícias mais precisas e atualizadas sobre ENEM, SISU, 
                        concursos públicos e educação em geral.
                      </p>
                      <p className="text-xs text-muted-foreground">
                        ✓ Validação por múltiplas fontes<br/>
                        ✓ Filtragem automática de qualidade<br/>
                        ✓ Atualização em tempo real
                      </p>
                      <div className="pt-2 border-t border-border">
                        <p className="font-semibold text-sm text-primary">🎯 Bônus de Contribuição</p>
                        <p className="text-xs text-muted-foreground leading-relaxed">
                          Ao clicar em "Mais notícias", você não apenas acessa conteúdo exclusivo, 
                          mas também contribui para enriquecer nossa base de dados com informações 
                          valiosas que beneficiam toda a comunidade educacional. Cada busca ajuda 
                          a melhorar nosso algoritmo de qualidade!
                        </p>
                      </div>
                    </div>
                  </TooltipContent>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="bg-muted/50 rounded-lg p-3 flex-1">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium">{userCount}/{userMaxContributions}</span>
                </div>
                <div className="flex items-center justify-between text-xs text-muted-foreground mt-1">
                  <span>Global hoje:</span>
                  <span>{globalCount}/{maxDaily}</span>
                </div>
                {remainingUsers > 0 && (
              ...
              <CardHeader className="pb-3">
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge 
                      className={`${getCategoriaColor(noticia.categoria)} text-white text-xs`}
                    >
                      {noticia.categoria}
                    </Badge>
                    {noticia.urgente && (
